...
public partial class WeekView : UserControl {
	...
	private const double PaddingX_WEIGHT = 1;
	private const double PaddingY_WEIGHT = 1;

	private double PaddingX => Bounds.Width * PaddingX_WEIGHT / (GRAPH_AREA_X_WEIGHT + 2 * PaddingX_WEIGHT);
	private double PaddingY => Bounds.Height * PaddingY_WEIGHT / (GRAPH_AREA_Y_WEIGHT + 2 * PaddingY_WEIGHT);
	...
	private double TimelineMarkHeight => GraphAreaHeight * 0.05;
	#endregion
	...
	//	double graphPosX = (task.start - TIME_INTERVALL_START_SECONDS) * proportion + PaddingX;
	...
	//		YAxisSegmentSize * (i % (MAX_TASKS / TASK_GRAPH_COLUMN_COUNT)) * 1.5 - additionalHeight + PaddingY,
	//		width,
	...
	private void DrawTimeline(DrawingContext context) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (i + 1 == (int)DateTime.Today.DayOfWeek)
				if (DateTimeService.FloorWeek((DataContext as WeekViewModel)!.CacheService.SelectedDay) == DateTimeService.FloorWeek(DateTime.Now))
	...
	//		FlowDirection.LeftToRight,
	//		new Typeface("Arial"),
	//		Math.Max(2.0, ArialHeightToPt(YAxisSegmentSize)),
